    ...
    if os.path.exists( code ):
       ...
       for var in fullVarList:
            if isinstance(var[1], int):
                taintTrace[var[1]] = var[0]
            elif isinstance(var[1], str) and ("," not in var[1]):
                taintTrace[var[1]] = var[0]
            elif isinstance(var[1], str):
                for subVar in var[1][1:].split(","):
                    taintTrace[subVar] = var[0]
        
       funcTrace = {}
       for declaration in funcDefList:
            funcTrace[declaration[2][-1]] = declaration[1]
       for call in call_list:
            taintTrace[call[2]] = funcTrace[call[3][-1]] 
       
       trace = [str(data)]
       while taintTrace.get(data):
         trace.append(str(taintTrace[data]))
         data = taintTrace[data]
       print("->".join(trace))
